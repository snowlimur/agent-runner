...
[ ] **T1.1**: Define version variable and create `VersionCommand` handler
...
[ ] **T1.2**: Wire `version` command into CLI dispatcher and update help text
...
[ ] **T1.3**: Update `Taskfile.yml` to inject version from git tag at build time
...
[ ] **T1.4**: Validate full build and all tests pass
